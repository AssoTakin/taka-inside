(function() {
  const config = window.__BENEVOLE_CONFIG__ || {};
  const requiredFields = config.requiredFields || ["lastName", "firstName", "email", "city", "skills", "motivation"];
  const successMessage = config.successMessage || "Votre candidature a bien été envoyée. Un email de confirmation vous a été envoyé.";
  const errorMessage = config.errorMessage || "Une erreur est survenue. Veuillez réessayer.";
  const submitLabel = config.submitButton || "Envoyer ma candidature";
  ...
  function init() {
    document.querySelectorAll('[data-group="skills"]').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.preventDefault();
        toggle('skills', btn.dataset.value, btn);
      });
    });
    ...
    document.querySelectorAll('[data-group="availabilities"]').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.preventDefault();
        toggle('availabilities', btn.dataset.value, btn);
      });
    });
