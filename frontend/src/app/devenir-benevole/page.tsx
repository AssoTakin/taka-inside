  ...
  return (
    <html lang="fr">
      ...
      <body className="font-body bg-taka-cream text-taka-black antialiased">
        ...
        <script dangerouslySetInnerHTML={{ __html: buildScript({ requiredFields, successMessage, errorMessage, submitButton }) }} />
      </body>
    </html>
  );
}
...
function buildScript(opts: { requiredFields: string[]; successMessage: string; errorMessage: string; submitButton: string }): string {
  return `
(function() {
  ...
  document.getElementById('benevole-form').addEventListener('submit', async function(e) {
    e.preventDefault();
