...
|| 2026-08-18 | **Pages Label Musical enrichies** : correction des liens section "Nos artistes" (cartes artistes → page individuelle, bouton rouge → page globale `/label-musical`). Refonte des pages artistes avec bio, cover, réseaux sociaux/streaming, actualités, discographie avec extraits audio, concerts, produits liés. Enrichissement du schéma Strapi `artiste` avec `slug`, `genre_musical`, `citation`, `photo_cover`, `liens_streaming`, `discographie`, `actualites`, `concerts`, `produits_lies`. Génération automatique des slugs `dj-kenza` et `mc-takin`. Sitemap mis à jour avec les pages artistes. |
|||

*Dernière mise à jour : 2026-08-18 (pages Label Musical enrichies + Resend)*