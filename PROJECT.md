...
| 2026-08-15 | **Liens/logos radio modifiables via Strapi** : ajout d'un champ `links` répétable au composant `homepage.radio-section`, basé sur `shared.cta-button`. Le frontend consomme cette liste et affiche les icônes Facebook/Instagram/X. Le backend bootstrap seede 3 liens par défaut si le champ est vide. La modification/ajout se fait dans Content Manager → Single Types → Page d'accueil → Sections → Radio Section → Liens. |
| 2026-08-15 | **Boutons Don/Bénévole uniformisés** : les deux boutons de la section Don + Bénévole ont la même largeur, hauteur et alignement (poussés en bas des cartes via `mt-auto`). La carte Bénévole est pilotée par `homepage.cta-benevole-section` plutôt que codée en dur. |
