...
| 2026-08-15 | **Régression homepage résolue** : les sections homepage (stats, projets en vedette, artistes, CTA don/bénévole) avaient disparu car `fetchHomepage()` utilisait un `populate` profond qui timeoutait sur Railway. Ajout d'un fallback `fetchHomepageLight()` pour récupérer les sections de base, et population complète des sections dans Strapi. Vérification visuelle OK. |
