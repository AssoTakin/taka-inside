...
|| 2026-08-18 | **Configuration Resend et envoi d'email bénévole validés** : remplacement du domaine Resend `solid-eat.com` par `takainside.org` (à restaurer : `solid-eat.com` était un projet en cours). Domaine `takainside.org` vérifié avec DKIM/SPF. Les emails transactionnels partent désormais depuis `benevole@takainside.org`. La notification admin arrive bien sur `benevole@takainside.org` (`delivered`). Les templates de confirmation candidat et de notification admin sont externalisés dans `formConfig.emailTemplates` du `page-content` `devenir-benevole` et modifiables depuis Strapi. Variables utilisables : `{{candidateName}}`, `{{firstName}}`, `{{lastName}}`, `{{email}}`, `{{phone}}`, `{{city}}`, `{{country}}`, `{{skills}}`, `{{availabilities}}`, `{{otherSkill}}`, `{{motivation}}`. Correction d'un bug UI où le message de succès ne s'affichait pas après soumission. Tests Playwright desktop + mobile repassent en production. |
|
